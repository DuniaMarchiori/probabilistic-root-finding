...
void iterative_probabilistic_rootfinding(uint32_t *factors, poly f1, int expectedDegree, int m) 
{
    ...
    unsigned char seed[2][size_random];
    uint maxCoeff = (1 << m);
...

        {
            do {
                do {
                    ...
                    randombytes(seed[1], size_random/2);
                    r = (gf) seed[0][0] | (gf)seed[0][1] << 8 | (gf)seed[1][0] << 16;
...
{    
	...
	for (i = 0; i < expectedDegree; ++i) {
        ...
        factors[i] = coeff ^ (!keepCoeff * UINT32_MAX);
    ...
    for (i = 0; i < expectedDegree; ++i) {	
        ...
        keepCoeff -= !(i - degOriginal);
      	x[i] = coeff ^ (!keepCoeff * (UINT32_MAX ^ fakeValue));
    }
